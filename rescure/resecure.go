...
var (
	dataDir string = "rescure_data"
	feedURL        = map[string]string{
		"Malicious IP Blacklist": "https://rescure.me/rescure_blacklist.txt",
		"Malicious Domain Blacklist": "https://rescure.me/rescure_domain_blacklist.txt",
		"Malware Hashes": "https://rescure.me/rescure_malware_hashes.txt",
	}
)

func Download(pathName string) {
	pterm.Info.Println("Rescure.me: Downloading Feeds")
...
func newThreatfetch(path string, feedURL map[string]string, dataDir string) *Rescure {
...
func dailyGenerationCheck(tf *Rescure) {

	// empty struct for missing files
	var missing_tf Rescure
